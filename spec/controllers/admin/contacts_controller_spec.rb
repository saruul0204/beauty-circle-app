...
describe Admin::ContactsController, type: :controller do # rubocop:disable Metrics/BlockLength
  ...
  let(:valid_attributes) do
    attributes_for(:contact).merge(full_name: contact.full_name, message: contact.message, email: contact.email)
  end
  ...
  describe 'GET index' do
    ...
    it 'filter Name exists' do
      get :index
      expect(filters_sidebar).to have_css('label[for="q_full_name"]')
      expect(filters_sidebar).to have_css('input[name="q[full_name_contains]"]')
    end

    it 'filter Name works' do
      matching_contact = create :contact, full_name: 'ABCDEFG'
      non_matching_contact = create :contact, full_name: 'HIJKLMN'

      get :index, params: { q: { full_name_or_email_cont: 'BCDEF' } }
  ...
  describe 'POST create' do
    context 'with valid params' do
      ...
      it 'creates the contact' do
        ...
        expect(contact.full_name).to eq(valid_attributes[:full_name])
  ...
  describe 'GET edit' do
    ...
    it 'renders the form elements' do
      expect(page).to have_field('Full name', with: contact.full_name)
  ...
  describe 'PUT update' do
    ...
    context 'with invalid params' do
      ...
      it 'does not change contact' do
        expect do
          put :update, params: { id: contact.id, contact: invalid_attributes }
        end.not_to(change { contact.reload.full_name })
      end
    end
  end
